...
class Board:
    ...
    def __str__(self):
        '''Allows printing the board to the console. It shows empty squares as [ ] and filled ones using the __str__ of the piece.'''
        s = '\n'
    ...
    def calc_moves(self, piece, row, col):
        ...
        def pawn():
            '''Pawn can move 2 squares forward only if it hasn’t moved before.'''
            ...
            # diagonal
            #Diagonal left/right positions. Checks if the square contains an opponent’s piece to capture.
        ...
        def knight():
            '''All 8 L-shaped knight moves.For each move, if it’s within the board and not blocked by an allied piece, it gets added.'''
            # possible moves
            possible_moves = [
            (row - 2, col + 1),
            (row - 1, col + 2),
        ...
        def straightline(incrs): #Bishop, Rook, and Queen
            '''Moves repeatedly in each direction until it hits a wall or a piece.'''
            for incr in incrs:
